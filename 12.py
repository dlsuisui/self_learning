...
dat_surg['RN1'] = dat_surg[['ADRG_pre','icd_code5', 'index0']].groupby(['ADRG_pre','icd_code5'])['index0'].rank()
dat_surg['RN2'] = dat_surg[['ADRG_pre', 'index0']].groupby(['ADRG_pre'])['index0'].rank()
...
def transExcel(path,data,merge_cols_list):
    ...
    for i in range(line_cn):
        if data['CN1'].tolist()[i] == 1:
            print('该行(ADRG_pre_icd)不存在需要合并的单元格')
            for j, col in enumerate(cols):
                # print(df.ix[i,col])
                worksheet2007.write(i + 1, j, data[col].to_list()[i], format_other)
        else:
            for j, col in enumerate(cols):
                if col in merge_cols[2:6]:  # 哪些列需要合并
                    if data['RN1'].to_list()[i]:  # 合并写第一个单元格，下一个第一个将不再写
                        # print(dat_diag['CN'].to_list()[i])
                        worksheet2007.merge_range(i + 1, j, i + int(data['CN1'].to_list()[i]), j,
                                                  data[col].to_list()[i], format_other)  ##合并单元格
                    else:
                        pass
                else:
                    worksheet2007.write(i + 1, j, data[col].to_list()[i], format_other)
...
if __name__=='main':
    ...
    merge_cols_list=['ADRG_pre','ADRG_name','icd_code5','count','counts','percent']
    #诊断输出
